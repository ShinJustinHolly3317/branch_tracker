...
app.get('/branches/suggest', async (req, res) => {
  ...
  pairs.sort((a, b) =>
    a.branchName.localeCompare(b.branchName, 'zh-Hant', { sensitivity: 'base' })
  )
...
app.get('/performance/branches', async (req, res) => {
  ...
  // Make sure we have forward prices: use an effective end date that is K days before latest.
  const endIdx = allDates.length - 1 - forwardDays
  if (endIdx < 0) {
    return res.json({
      startDate: '—',
      endDate: '—',
